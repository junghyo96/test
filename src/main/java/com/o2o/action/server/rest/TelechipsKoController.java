...
@RestController
public class TelechipsKoController {
	private final TelechipsKo telechipsKo;

	@Autowired
	private CategoryRepository categoryRepository;

	public TelechipsKoController() {
		telechipsKo = new TelechipsKo();
	}

	@RequestMapping(value = "/telechipsKo", method = RequestMethod.POST)
	public @ResponseBody String processActions(@RequestBody String body, HttpServletRequest request,
		...
		try {
			...
			jsonResponse = telechipsKo.handleRequest(body, CommonUtil.getHttpHeadersMap(request)).get();
